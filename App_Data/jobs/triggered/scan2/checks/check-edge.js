/**
 * Description: Checks if browser is in Edge mode by checkin the doctype of website and the compat
 * list and meta tags.
 *
...
var check = function (website) {
    ...
    if (compatTag.length > 0) {
        if (compatTag[0].attribs.content) {
            ...
            if (mode.indexOf("edge") === -1) {
                result.passed = false;
                result.data = { source: 'tag', mode: mode};
    ...
    cvlist.getList().then(function (list) {
        ...
        if (resultWebsite && (resultWebsite.docMode || resultWebsite.uaString || Object.keys(resultWebsite).length === 0)) {
            result.passed = false;
            result.data = {source: 'cvlist', mode: resultWebsite.docMode};
        }
