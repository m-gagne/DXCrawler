...
function determineProgress(lines) {
	...
	if (useazureassource) {
		var blobSvc = azure.createBlobService(config.storage_account_name, config.storage_account_key);
		console.log('reading blob', progressResultsFile);
		blobSvc.getBlobToText(config.website_list_container_name, progressResultsFile, function (err, text, blockBlob, response) {
			if (err) {
				console.log(err);
...
function doLines(lines, progress) {
	...
	for (var progressCounter = 0; progressCounter < progress.length; progressCounter++) {
		progress[progressCounter] = prefix + progress[progressCounter];
	}
...
function doWork(websites, progress) {
	...
	function processData(data) { 
		...
		try {
			...
			if (!!nrows && nrows % DUMP_RESULTS == 0) {
				...
				if(newProgress)
					saveDataToFile(progressResultsFile, newProgress);
	...
	batch.onFinish = function () {
		...
		saveDataToFile(progressResultsFile, newProgress, function () {
			if (useazureastarget) {
				// Remove local file
				fs.unlinkSync(progressResultsFile);
			}
		});
