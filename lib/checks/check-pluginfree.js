...
function getOptions() {
    ...
    var configfile = path.join(__dirname, 'check-pluginfree.json');

    if (fs.existsSync(configfile)) {
        console.log('Reading plugin free options');
...
var initiate = function (website, options) {
    ...
    cvlist.getList().then(function (list) {
            ...
            // allows SVG and SWF and Silverlight and disallow everything else
                var $objects = website.$('object'),
                    ...
                    $objectTypeSilverlight = website.$('object[type*=silverlight]'),
                    $embedsSWF = website.$('embed[src*=swf]'),
                    $embedsSVG = website.$('embed[src*=svg]'),
                    $embedsSilverlight = website.$('embed[type*=silverlight]');
                    
                var activeXcontrols = $objects.length - $objectParams.length - $objectSWFparams.length - $objectSVGparams.length - $objectTypeSilverlight.length
                                    + $embeds.length - $embedsSWF.length - $embedsSVG.length - $embedsSilverlight.length;

                var flashControls = $objectParams.length + $objectSWFparams.length + $embedsSWF.length + $embedsSVG.length;
                var silverlighControls = $objectTypeSilverlight.length + $embedsSilverlight.length;
                
                if (flashControls > 0 && !options.allowFlash) {
                    test.passed = false;
                    test.data = {activex: true, cvlist: false, flash: true};
                }
                else if (silverlighControls > 0 && !options.allowSilverlight) {
                    test.passed = false;
                    test.data = {activex: true, cvlist: false, silverlight: true};
                }               
